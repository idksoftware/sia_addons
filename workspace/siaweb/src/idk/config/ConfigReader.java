...
public class ConfigReader {
	...
	static public boolean read(final String homePath) {
		...
		ConfigInfo.setRootPath(homePath);
	...
	static public boolean doFolderCheck() {
		...
		File file = null;
		String rootPath = ConfigInfo.getRootPath();
		...
		String usersPath = ConfigInfo.getUsersPath();
		file = new File(usersPath);
		if (file.exists() == false) {
			errorMessage = "SEVERE Error: Can not find the users folder \""
					+ usersPath + "\"";
			return false;
		}
	...
	static void parseFile(final Element root) throws ParseException {
		final String rootPath = XMLReadHelper.stringValue(root, "RootPath", true);
		ConfigInfo.setRootPath(rootPath);
		...
		final String usersPath = XMLReadHelper.stringValue(root, "UsersPath", true);
		ConfigInfo.setUsersPath(usersPath);
		...
		// String webSitePath = XMLReadHelper.stringValue(root, "WebSitePath",
		// true);
		// ConfigInfo.setUsersPath(webSitePath);

		final String versionTypeStr = XMLReadHelper.stringValue(root, "AppMode", true);
		try {
			final VersionType versionType = VersionType.valueOf(versionTypeStr);
			ConfigInfo.setVersionType(versionType);
		} catch (final IllegalArgumentException e) {
			throw new ParseException(e.getMessage(), 0);
		}

		final String singleUserName = XMLReadHelper.stringValue(root, "SingleUserName", true);
		ConfigInfo.setSingleUserName(singleUserName);
		/*
