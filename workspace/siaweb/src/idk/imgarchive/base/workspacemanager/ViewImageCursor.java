 ...
 * 
 */
public class ViewImageCursor implements Iterable<ImageAddress> {
	...
	long imagesPerPartition = 0;
	String root = null;

	public ViewImageCursor() {
		imagesPerPartition = ConfigInfo.getImagesPerPartition();
		root = ConfigInfo.getInstanceWWWPath();
	}
	...
	private boolean changeCurrentAddress() {
		...
		final long partition = findPartition(currentIndex.current);

		final String imagePath = root + File.separator + ImageAddress.makeAddressString(partition) + File.separator
	...
	final long findPartition(final long idx) {
		final long idxOffset = idx - 1;
