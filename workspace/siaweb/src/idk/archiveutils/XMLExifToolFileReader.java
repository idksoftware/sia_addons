 ...
 */
public class XMLExifToolFileReader {
	
	private static final int MAX_ATTRIB = 2;
	private static final int NAME_SPACE = 1;
	private static final String XMLNS_ABOUT = "rdf:about";
	...
	public XMLExifToolFileReader(final File ifile, final SummaryMetadata summaryMetadata) {
		inputFile = ifile;
	...
	public void process() throws IOException, ParseException {
		...
		final NodeList rdf = root.getChildNodes();
		// NodeList rdfs = root.getElementsByTagNameNS(
		// "http://www.w3.org/1999/02/22-rdf-syntax-ns#", "*");
		int n = rdf.getLength(); 
		for (int i = 0; i < rdf.getLength(); i++) {
			final Node node = rdf.item(i);
			System.out.println(XMLReadHelper.nodeType(node.getNodeType()));
			...
			for (int ns = 0; ns < nmap.getLength(); ns++) {
					String nameSpace = nmap.item(ns).getNodeName();
					System.out.println(nameSpace);
					//nmap.item(ns).
			}
			final NodeList noteList = node.getChildNodes();
			for (int j = 0; j < noteList.getLength(); j++) {
				System.out.println(XMLReadHelper.nodeType(node.getNodeType()));
				if (node.getNodeType() == 3) {
					continue;
				}
				String name = noteList.item(j).getNodeName();
				String content = noteList.item(j).getTextContent();
...
/*
			if (nameSpace.matches(XMLNS_TIFF)) {
				xmlnsTiff(node);
				continue;
			} else if (nameSpace.matches(XMLNS_ABOUT)) {
				continue;
			...
			for (int j = 0; j < childNodes.getLength(); j++) {
				...
				if (cnode.getNodeType() == ELEMENT_NODE) {
					...
					if (name.matches("aux:SerialNumberText") == true) {
						//summaryMetadata.getAssetPropertiesDirectory().caption = text;
					}
					if (name.matches("aux:LensInfoText") == true) {
						//summaryMetadata.getMediaPropertiesDirectory().orientation = text;
					}
